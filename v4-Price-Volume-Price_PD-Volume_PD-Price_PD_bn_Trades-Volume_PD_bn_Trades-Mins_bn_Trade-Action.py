...
def chunk_data(price_lst, volume_lst, price_pd_lst, volume_pd_lst, price_pd_bn_trades_lst, volume_pd_bn_trades_lst, mins_bn_trade_lst, act_lst, export_file_name):
	r=''

	title = 'Price,Volume,Price_PD,Volume_PD,Price_PD_bn_Trades,Volume_PD_bn_Trades,Mins_bn_Trade,Action\n'
	r += title
	for i,val in enumerate(price_pd_lst):
		if(i <= len(price_pd_lst)):
			r += '{},{},{},{},{},{},{},{}\n'.format(price_lst[i], volume_lst[i], price_pd_lst[i], volume_pd_lst[i], price_pd_bn_trades_lst[i], volume_pd_bn_trades_lst[i], mins_bn_trade_lst[i], act_lst[i])
